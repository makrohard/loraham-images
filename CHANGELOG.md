...
- Rebuild on `loraham-pi-control` **v0.4.1** (`97fd1c4`); see that repo's changelog.
...
- Rebuild on `loraham-pi-control` **v0.3.10** (`dce430f`): Voice is an ordinary pinned source, so
  an install lands on and verifies the manifest pin instead of the branch tip. Source pins and
...
- Rebuild on `loraham-pi-control` **v0.3.5** (`b4ad26a`): test-suite rework only — no product
  behaviour change. Source pins and binaries unchanged since v0.3.1.
